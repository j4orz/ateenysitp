...
By the end of [Part II. Neural Networks](), we will be one step closer in achieving our quest of building our own **[ChatGPT]()**
...
After completing part two, you will be ready for [Part III. Scaling Networks](./3.md) of the book where we finally achieve our quest of building our own **[ChatGPT]()**.
...
human feedback<span class="sidenote-number"></span><span class="sidenote">*Originally presented to the world in [Introducing ChatGPT (OpenAI 2022)](https://openai.com/index/chatgpt/), and reproduced by open source in [Llama 2: Open Foundation and Fine-Tuned Chat Models (Touvron et al. 2023)](https://arxiv.org/pdf/2307.09288)*</span>
...
- [4. Learning *Sequences* via Deep Neural Networks with `teenygrad`]()
  - [4.1 From Supervised to Self-Supervised Learning]()
  - [4.2 Learning Sequences with Linear Models]() <!--  hmm.... -->
  - [4.3 From Linear to Non-Linear Learning]()
...
- [5. Accelerating *Sequence Models* on `GPU` in `teenygrad`]()
  - [5.1 From Numerical Linear Algebra Libraries to Deep Learning Frameworks]()
  ...
  - [5.4 From `SIMD` to `SIMT`: From Multi Core to Many Core Processors]()
  - [5.5 Accelerating `GEMV` on `GPU` with `CUDA Rust` and `PTX` via Rooflines]()
  - [5.6 Accelerating `GEMM` on `GPU` with Data Reuse]()
  - [5.7 Accelerating `GEMM` on `GPU` with Scheduling]()
...
```python
{{#include ../../teeny/examples/4.2-makemore-ffn1layer-torch.py:42:69}}
```

```python
{{#include ../../teeny/examples/4.2-makemore-ffn1layer-torch.py:77:}}
```
