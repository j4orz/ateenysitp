...
so before defining the former we must define the latter, which corresponds to the picture below,
displaying the idea that **probability is a measure of the size of sets**.

<img class="probspace" src="./assets/probspace.png" alt="" style="display: block; margin: 0 auto;">

This picture is comprised of three components. Namely, the blue box and it's blue dots, the colored boxes, and the mapping from the colored boxes to the $[0,1]$ number line
...
something to say here about event spaces? cardinality of them?
...
Probability distributions are the `np.ndarray` objects we used throughout [§1. Sequence Learning](#1-sequence-learning)
in the implementations of the bigram and logistic regression language model.
So how do these connect to the concept of probability spaces just defined?
The answer is with **random variables**, corresponding to the picture below,
which displays how **random variables map outcomes to numbers**.

<img class="randvar" src="./assets/randvar.png" alt="" style="display: block; margin: 0 auto;">

This picture shows the same sample space $\Omega$ and event space $\mathcal{F}$ from [§I1.1 Probability Spaces](#i11-probability-spaces), but instead of a probability law $\mathbb{P}: \mathcal{F} \to [0,1]$
mapping events to the [0,1] number line, there is a random variable $X: \Omega \to \reals$
mapping **outcomes to the real number line $\reals$**.
