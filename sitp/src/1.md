...
in Figure 1.1 are the top 10 most likely.
...
and with Figure 1.1, you are asking GPT-2 to produce $p(w| \text{Hello nice to})$.
In other words, you are asking GPT-2 *given* that the sentence starts with $\text{Hello nice to}$,
what are the probabilities you assign to possible next words?
So for $p(w|\mathbf{h})$ more generally, there is *no* randomness associated with the history $\mathbf{h}$.
...
So how do these connect to the abstract concept of probability spaces we just defined?
...
the `np.ndarray` output from GPT-2 when passed the input string `"Hello nice to"`:
...
How is `probs` related to the abstract concept of a probability space? 
...
Notice the level of indirection with the random variable. All we've done is translate from sample space to state space.
To obtain probabilities, we can map from the state space $\reals$ to probabilities $[0,1]$ with the probability mass function $p_X(x): \reals \to [0,1]$.
That is,
...
to obtain $p_X \circ X: \Omega \to [0,1]$. That is, evaluating $p(X(\omega))$ results in the probability of outcome $\omega$.
(todo. this is wrong?) The final probabiltiy mass function is represented in Python with `numpy`'s `np.ndarray` `probs`<span class="sidenote-number"></span><span class="sidenote">*For conveniece sake we do store the outcomes in the `tokens` array so that we can print them next to their probabilities by looping over states $x \in \reals$ with `for i in np.argsort(-probs)`, but it's important to note that $p_X(x)$ is defined on real numbered states (represented with indices), and *not* on set-valued events.*</span>. That is,
...
simply slugging around probability mass functions with `np.ndarray`s.
