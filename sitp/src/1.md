...
- [1. Sequence Learning](#1-sequence-learning)
  ...
  - [1.2 Estimation with Counting of Bigrams](#12-estimation-with-counting-bigrams)
  - [1.3 Parameterization with Iterative Fitting Logistic Regression](#15-iteratively-fitting-logistic-regression-with-cross-entropy-loss) <!-- logistic reg/1 layer ffn -->
  ...
  - [1.7 Further Reading](#18-bibliographic-notes)
...
Practically, it's predominantly because of the fossil-fuel like <span class="defnote">**data**</span>**data** subsidy provided by the aforementioned
...
In other words, you are asking GPT-2 *given* that the sentence starts with $\text{Hello nice to}$,
what are the probabilities it assigns to possible next words?
That is, with $p(w|\mathbf{h})$ more generally, there is *no* randomness associated with the history $\mathbf{h}$.
We can print the conditional distribution that GPT-2 produces given the history $\text{Hello nice to}$, and verify that 1. each probability is between 0 and 1 and 2. the distribution normalizes to 1
...
with 1-0.8181=0.1819 spread amongst the other 50256-10=50246 unseen words. You may have also noticed
...
While we will come to be intimately familiar with mutldimensional arrays such as `np.ndarray` by the end of the book,
...
enabling the representation of scalars, vectors, matrices, and arbitrary tensors of arbitrary rank.
In the case of `probs`, it's a vector with support in $\reals^{10}$,
...
Another important property is `ndarray.ndim`, which is `len(probs.shape)` and reports the rank of an array. Since `probs.ndim` evaluates to 1, it has a rank of 1, or equivalently, is a vector.

> [!WARNING]
> <img class="owl" src="./assets/owl.png" alt="" style="float: left; width: 100px; margin: 0 1em 0.5em 0;"> A 10-dimensional vector is some $\mathbf{x}^{(1)} \in \reals^{10}$. A 100-dimensional vector is some $\mathbf{x}^{(2)} \in \reals^{100}$. Both vectors $\mathbf{x}^{(1)}$, and $\mathbf{x}^{(2)}$, have a rank of 1, hence why they are both considered vectors. Unfortunately with numpy a rank is stored under the property `ndarray.ndim`. <br><br>So, an array whose `ndarray.ndim` evaluates to 2 is not some vector $\mathbf{x}^{(3)}\in \reals^2$ but rather, an array that is some matrix $\mathbf{x}^{(4)} \in \reals^{n\times m}$, where $n$ and $m$ are unknown since `ndarray.shape` was not supplied. Conversely, a multidimensional array is not simply a flat array with arbitrary length $n$ to represent any vector in $\reals^n$, but rather an array with arbitrary rank $n$ to represent any tensor $\mathbf{x}^{(4)}\in\reals^{d_1 \times d_2 \times \cdots \times d_n}$. **That is, multidimensional arrays are more accurately described as multirank arrays!** <br style="clear: both;">

Returning to the focal point of probability and GPT-2's conditional distribution $p(w|\mathbf{h})$,
we now know that it can be represented with an `ndarray` whose `.shape` is `(50256,)`,
...
which represented a vector $\mathbf{x} \in \reals^{10}$.
The first important principle of [§1. Sequence Learning]()
is that **probability can be seen as array programming**.

> [!IMPORTANT]
> <img class="owl" src="./assets/owl.png" alt="" style="float: left; width: 100px; margin: 0 1em 0.5em 0;"> **Probability can be seen as array programming**! <br style="clear: both;">
...
lorem...<span class="defnote">**probability of AND**</span>**probability of AND**
ipsum..<span class="defnote">**sum rule**</span>**sum rule**
...
### 1.2 Estimation with Counting Bigrams
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
of a sentence's next word given those that have already occured, $p(w|\mathbf{h})$.
For instance, with the input text `"Hello nice to meet"` as the history $\mathbf{h}$,
GPT-2 produces the following distribution represented with an `ndarray` of `.shape` `(10,)`.
...
We now shift our attention into implementing functions like GPT-2 that can produce such a conditional distribution $p(w|\mathbf{h})$.
We will incrementally increase the expressivity of our models culiminating with GPT-2's transformer architecture in [Part II. Deep Neural Networks](./2.md). For now, we start with the basics and return to first principles.

As mentioned in the previous chapter, large language models are trained using methods from the discipline of <span class="defnote">**deep learning**</span>**deep learning**, which in turn is based in <span class="defnote">**machine learning**</span>**machine learning**,
which itself is founded in statistics.
Generally speaking, probability is about generating data by evaluating the distribution $p(w|\mathbf{h})$
whereas statistics is about recovering such a distribution $p(w|\mathbf{h})$ *from* observed data.
As we will see, the nature of training language models is much more akin growing bacteria in biology
(albeit with the precise tools of mathematics) rather than the classic way of programming software 1.0, line by line.
So how do we recover the distribution $p(w|\mathbf{h})?$



- definition of conditinal probability from joint
- global vs local distinction.
- distribution over sentences vs words
...
- introduce random variables in prev section? $p(W_t=w_t| W_1=w1, \cdot W_{t-1}=w_{t-1})$
...
### 1.3 Iteratively Fitting Logistic Regression  <!-- with Cross Entropy Loss $\mathscr{L(\theta)} := $ -->
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
### 1.4 Directly Fitting Linear Regression  <!-- with Least Squared Error -->
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
But how is the multidimensional array `probs` with support in $\reals^{10}$ related to the abstract concept of a probability space?
