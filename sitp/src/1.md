...
The quintessential example for this approach is ChatGPT. **How is it possible for us to go from systems like ELIZA to those of ChatGPT in just half a century?**
...
begins and ends by explaining the inner workings of the chatbot at a high level (emphasis mine):
...
So at a high level ELIZA interprets language by pattern matching over certain keyword strings and then producing a response based on predefined reassembly rule. Conveniently, Weizenbaum provides a more concrete example later in the paper:
...
Besides providing a concrete example of the *method* in which ELIZA interprets and produces language (with pattern matching), what's illuminating with respect to the *structure* in which ELIZA represents language is not only that the foreigner has a *limited* understanding of the English vocabulary, but also more importantly that the foreigner is simply representing language with symbolic strings. The foreigner in this example, and also ELIZA itself, is effectively a **pattern matcher over strings**. This is no different from the traidition in philosophy which represents word meaning by simply spelling the word with capital letters. An illustrative example is provided by linguist Barbara Partee (1967) where the meaning of life is (represented with) the *string* `"LIFE"`.

> [!TIP]
> Try prompting the ELIZA embedded up above with what the meaning of life is.<br> Do you think ELIZA understands?

Although ELIZA was implemented using decomposition and reassembly rules in the SLIP programming language<span class="sidenote-number"></span><span class="sidenote">*Weizenbaum was a Schemer, for he also was the creator of the SLIP programming language itself, presented in [Symmetric List Processor (Weizenbaum 1963)](https://dl.acm.org/doi/pdf/10.1145/367593.367617).*</span>, those rules are quite similar to those of **regular expressions**<span class="defnote">**regular expressions**</span>. Let's reproduce some of the example scripts presented in the paper by re-implementing it's pattern-match-over-string functionality with Python.
...
def eliza(input: str) -> str:
  for pattern, responses in transformation_rules:
    if m := re.fullmatch(pattern, input, re.IGNORECASE): jreturn random.choice(responses).format(*m.groups())
...
Although we have only implemented a small fraction of the functionality<span class="sidenote-number"></span><span class="sidenote">*The curious reader whom wishes to explore more transformation rules should play around with the embedded ELIZA up above, or take a look at the Appendix in [(Weizenbaum 1963)](https://dl.acm.org/doi/pdf/10.1145/365153.365168).*</span> (albeit the original ELIZA program is only a few hundred lines of code itself), it illustrates the discrete structures and deterministic methods used to implement ELIZA,
and specifically with the last transformation rule, how brittle ELIZA's so-called "understanding" truly is.
The primary reason why a simple pattern matcher over strings can be endowed with human "understanding" is simply the psychiatric context, where users are effectively talking with themselves:
...
understood that ELIZA falls under the former.

Returning to ELIZA, more coloquially the system can be described as "all style no substance":
...
Well, before jumping ship from software 1.0's good old fashioned discrete structures like strings and deterministic procedures like transformation rules, perhaps to build a system like ChatGPT all we need to do is increase the expressivity of such structures and procedures?

---

#### Associations and Hierarchies, and Networks, Oh My!
...
#### To Understand Claude We Must Return to Claude
