...
What that looks like in practice is simply using conditional statements.
...
(jeff, thursday, present regular expressions following jurafsky, perhaps pulling in some formal language theory)
...
understood that ELIZA is closer to the former.

(jeff thursday, present the embodied/symbolic of formal semiotics/semantics)
...
Following the last quote from the Weizenbaum paper, ELIZA was indeed not simply a one to one a translating processor however one could produce a few sentences given one (1:N). 
