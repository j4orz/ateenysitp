...
what the two intellectual traditions share in common is that their original conceptions focused on the activity of describing reality with *discrete objects* and *deterministic descriptions*.
However, the programmer interested in learning the skills required to build artificial intelligence
must make the transition that has already been made a few centuries ago by our mathematical cousins; the transition towards describing reality with *continuous objects* and *stochastic descriptions*.
...
— for example, to name a few, language translation with Google Translate, speech recognition with Siri, vision with Tesla Autopilot —,
...
However, rather than being implemented in a language like C, Java, or Javascript, it's implemented in another one called **PyTorch**,
a software 2.0 programming language humbly embedded in a Python package.

In this whirwild tour dubbed *The Structure and Interpretation of Tensor Programs*, we will embark on a quest to build our very own **ChatGPT** and **PyTorch** from
...
by following our mathematical cousins and make the transition from the *deterministically discrete of software 1.0* to the *stochastically continuous of software 2.0*.
Rather than replicate these massive industrial systems,
we will implement a teaching chat assistant [nanochat](http://github.com/karpathy/nanochat)
and a teaching deep learning framework [https://github.com/j4orz/teenygrad]()
...
so that the only prerequisite required is solid grasp with the art of programming, and high school algebra.
...
<!-- what drives the linearization/topological graph of the curriculum?
- the goals/skills (dcic). not the concepts/methods/machinery (htdp)

Part I
  0. need anything before this?

  1. learn the representation (embeddings) with supervised learning
    -> whatever mathematical machinery needed
    basic LA:
    basic prob:
    basic linear modeling (Nor, Bin, Exp)?
    TODO: can you introduce linear modeling machinery in order to fulfill the goal of representation learnign?
           
  2. goal: dimensionality reduce the representation with unsupervised learning
    -> whatever mathematical machinery needed
    adv LA (singular values, eigen values)

  *take note that LA machinery/concepts/defs will be splat across two chapters
   that is the cost of using motivating goals as the primary driver.

   it's not just "ch1: prelims", all defs

  * take note that it *is* data and then function but with respect to the high level goals, not the methods
    first we representation learn the embeddings. but this means but this means supervised learning of {x^(i), y^(i)} <------------------ TODO
    then we perform an operation on the embeddings, namely dim_reduction(embeddings). but this means unsupervised learning of {x^(i)}

    so we are presenting D={x^(i), y^(i)}, and then D={x^(i)} <- not the other way around
...
- [1. Syntactic Sequence Learning via n-gram Models with `teenygrad`]()
  - [1.1 From Deterministic Propositions to Stochastic Distributions]()
...
- [3. Accelerating Numerical Linear Algebra in `teenygrad`]()
...
## 1. Elements of Tensor Programs <!-- $p(X=x)$ from $D=\{x^{(i)}\}_{i=1}^{n}$ -->
...
### 1.1 From Software 1.0 to 2.0: Propositionally Discrete to Probabilistically Dimensional
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
...
Building machines which can understand and produce natural language has been a long standing dream of artificial intelligence.
After the physical realization of abstract turing machines in the form of digital computers, humanity continually increased the level of abstraction of our formal languages used to direct computation.
Programs were intially implemented with machine languages, then assembly languages, with systems languages, and finally high level languages which were so high, that they started feeling English-like.
But something was always missing.
Formal languages on the other hand such as programming languages are rigid, sharp, and precise, allowing mechanical specification.
Howevever natural languages are fundamentally flexible, fuzzy, and imprecise, allowing easy evolution.
How do we build machines that can understand and produce the latter?
...
by programming discrete structures of sets, lists, trees, graphs, associations with deterministic descriptions of assignment, control flow, and functions.
A classic exemplar for this approach is ELIZA, a computer chatbot which simulates a Rogerian psychotherapist

Ultimately however, this approach ran into limitations, and over the years fell out in favor of a different approach
which wanted to build intelligent machines by programming continuous structures of scalars, vectors, matrices, tensors with stochastic descriptions of probabilities, random variables, and distributions.
The most recent example is clearly that of ChatGPT, which took the world by storm in 2022. Above, we use an open source large language model.
...
let's first review the kind of methods and their limitations which natural language processing systems such as ELIZA used.These methods are now known as <span class="defnote">**GOFAI**</span> good old fashioned artificial intelligence, or <span class="defnote">**software 1.0**</span> **software 1.0**.

---

Let's take a look together at the original paper that presents ELIZA
...
which begins and ends by explaining the inner workings of the chatbot at a high level:

> The gross procedure of the program is quite simple. The text is read and inspected for the presence of a keyword. If such a word is found, the sentence is transformed according to a rule associated with the keyword, if not a content-free remark or, under certain conditions, an earlier transformation<span class="sidenote-number"></span><span class="sidenote">*The word "transformation" is used in its generic sense rather than that given it by Harris and Chomsky in linguistic contexts. (Ignore this side note if it doesn't mean anything to you).*</span> is retrieved. The text so computed or retrieved is then printed out. <br><br>Input sentences are analyzed on the basis of decomposition rules which are triggered by key words appearing in the input text. Responses are generated by reassembly rules associated with selected decomposition rules (...) The ELIZA program itself is merely a translating processor in the technical programming sense.

The paper effectively states that ELIZA is a **syntactic**<span class="defnote">**syntax**</span> pattern matcher over strings without much **semantic**<span class="defnote">**semantics**</span> understanding.
The classic illustrative example provided by linguist Barbara Partee (1967) is where the meaning of life is (represented with) the *string* `"LIFE"`.
Weizenbaum even goes on to describe the interpretation and production of English by a foreigner who lacks fluency (emphasis mine):

> Consider the sentence "I am very unhappy these days". Suppose a foreigner with only a limited knowledge of English but with a very good ear heard that sentence spoken but understood only the first two words "I am". Wishing to appear interested, perhaps even sympathetic, he may reply "How long have you been very unhappy these days?" <br><br>What he must have done is to apply a kind of *template* to the original sentence, one part of which matched the two words "I am" and the remainder isolated the words "very unhappy these days". He must also have a *reassembly* kit specifically associated with that template, one that specifies that any sentence of the form "I am BLAH" can be *transformed* to "How long have you been BLAH", independently of the meaning of BLAH. A somewhat more complicated example is given by the sentence "It seems that you hate me". Here the foreigner understands only the words "you" and "me"; i.e., he applies a template that decomposes the sentence into the four parts: <br><br>(1) It seems that (2) you (3) hate (4) me<br><br> of which only the second and fourth parts are understood. The reassembly rule might then be "What makes you think I hate you"; i.e., it might throw away the first component, translate the two known words ("you" to "I" and "me" to "you") and tack on a stock phrase ("What makes you think") to the front of the reconstituted sentence.

Although ELIZA was implemented using
decomposition and reassembly rules in the SLIP programming language<span class="sidenote-number"></span><span class="sidenote">*Weizenbaum was a Schemer, for he also is the creator of the SLIP programming language, presented in [Symmetric List Processor (Weizenbaum 1963)](https://dl.acm.org/doi/pdf/10.1145/367593.367617).*</span>, those rules are quite similar to those of **regular expressions**<span class="defnote">**regular expressions**</span>.

(some regexp code stuff)

```python

```

> Gorn [2] in a paper on language systems says: <br><br> Given a language which already possesses *semantic content, then a translating processor, even if it operates only syntactically, generates corresponding expressions of another language to which we can attribute as "meanings" (possibly multiple — the translator may not be one to one) the "semantic intents" of the generating source expressions; whether we find the result consistent or useful or both is, of course, another problem. It is quite possible that by this method the same syntactic object language can be usefully assigned multiple meanings for each expression... It is striking to note how well his words/fit ELIZA. The "given language" is English as is the "other language", expressions of which are generated.<span class="sidenote-number"></span><span class="sidenote">*Weizenbaum also conveys the same conclusion to the public in a [1978 interview](https://www.youtube.com/watch?v=GxSJQnWzJOs): "Well, I would deny that that there's any important sense, non-negligible sense in which the program understands. It certainly creates the illusion of understanding. there's no question about that. But we have to understand that that illusion is an attribution that the person conversing with the program contributes to the conversation. It's not a function of the program itself."*</span>

Can we do better than translating processors pattern matching on strings of the English language?
Perhaps we just need discrete structures and algorithms which are more expressive?

---

Associations, Hierarchies, and Networks Oh My

**dictionaries**

Clearly, there is something left to be said about this representation of word meaning, namely, that this representation of word meaning is not amenable to encoding
multiple meanings, concepts, or <span class="defnote">**sense**</span>**senses** with a single word <span class="defnote">**lemma**</span>**lemma**.
This is one diserata that we require our representation to fulfill,
since words we use on a daily basis can change their meaning depending on the context.
For instance, according to the Merriam Webster dictionary<span class="sidenote-number"></span><span class="sidenote">*The same activity can be with the Cambridge and Oxford dictionaries. Pick your favorite!*</span>, a few meanings or senses of the word "life" include:
...
(i.e the meaning of life is trivially `"LIFE"`), perhaps we can use a *dictionary* data structure, whose etymology is *literally* derived from word dictionaries?
Rather than seed our own dictionary, we can use *WordNet*, available through the Natural Language Toolkit Python package `nltk`.
Let's lookup the definition of the "life" lemma:
...
<!-- ### 1.5 Learning Probabilities from Data via Maximum Likelihood Estimation
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small> -->
...
```python
{{#include ../../teeny/examples/1.6-vspace.py:112:}}
```
