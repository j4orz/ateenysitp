...
So with all that said, go on young hacker. Venture forth! Do not think too much, and just do.
...
- [1. Syntactic Sequence Learning via n-gram Models with `teenygrad`]()
  - [1.1 From Software 1.0 to Software 2.0](#11-from-software-10-to-software-20)
  - [1.2 Distributing Data *Stochastically* with Probabilities](#12-distributing-data-stochastically-with-probabilities)
  - [1.3 Composing Probabilities with Rules: Sums, Products, and Conditionals](#13-composing-probabilities-with-rules-sums-products-and-conditionals)
  - [1.4 Random Variables with their Distributions, Expectations, and Variance]()
  - [1.5 Learning Probabilities from Data with Parameters via Maximum Likelihood Estimation]()
...
> [!NOTE]
> Although you may have used regular expressions throughout your day to day programing with text processing program such such as `sed`, `awk`, and `grep`, historically speaking they were originally conceived by Stephen Kleene to formally define **regular languages** in formal language theory with applications in linguistics, mathematics, and computer science, especially in compiler construction with respect to lexing and parsing during syntactic analysis. <br><br> But, although we are embarking on a quest to re-implement a Tensor compiler like **PyTorch** with **teenygrad**, because the language is embedded into Python, no text parsing is actually required. However, interestingly enough, regular expressions are in fact used to build **ChatGPT** and subsequently **nanochat** in what is called **tokenization**. Things come full circle as Stephen Kleene was originally motivated to describe the artificial neural networks of McCullogh and Pitts. <br><br>All this to say, is that besides illustrating ELIZA's GOFAI rules-based approach with regular expressions, becoming somewhat familiar with them will also help you with **ChatGPT** and **nanochat**'s tokenization process.

Python's [`re`](https://docs.python.org/3/library/re.html#module-re) module
is a C extension module (so not part of Python's language spec). 

with [`re.fullmatch`](https://docs.python.org/3/library/re.html#re.fullmatch)
which returns a correspondong [`re.Match`](https://docs.python.org/3/library/re.html#re.Match)
...
That is, perhaps we should find a representation so that the meaning of life is not simply `"LIFE"`.
...
linguistic understanding of meaning.
- denotation and connotation
- signifier and signified (semiotics with signs. in the case of semantics, with words)
...
### 1.3 Composing Probabilities with Rules: Sums, Products, and Conditionals
<small>[$\hookleftarrow$ Table of Contents](#i-elements-of-networks-1)</small>
