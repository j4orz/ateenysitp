...
We can do this in Python with regular expressions — accessible via standard library's [`re`](https://docs.python.org/3/library/re.html) module —
...
which returns a corresponding [`re.Match`](https://docs.python.org/3/library/re.html#re.Match) object or `None`. For example:
...
you can evaluate the `re.Pattern` and `re.Match` objects with a single function invocation rather than two:
...
Remember that entire purpose of regular expressions was to collapse the space we were enumerating through with conditionals,
so at least there is at least some reuse, for instance, with all prompts that take the form of "I am BLAH".
This is done via **capture groups**:
