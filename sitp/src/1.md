...
using Lean, Python, Rust, and CUDA Rust.
After our journey together, we encourage you to modify, extend, and hack on it.
...
- [Overture: A Lean Snake and Parallel Crab](#overture-the-language-of-logic-sets-and-functions)
- [1. Sequence Learning](#1-sequence-learning)
  ...
  - [1.2 Language as a Probability Space of Sequences](#12-language-as-a-probability-space-of-sequences)
  - [1.3 From Variables to Random Variables](#13-from-variables-to-random-variables)
  - [1.4 The Inference of Parameter Estimation]() <!-- ngram -->
  - [1.5 Iteratively Fitting Logistic Regression]() <!-- logistic reg/1 layer ffn -->
...
That is, a "how to read this book" if you will, explaining how concepts will be presented and explained to you dear reader.
</div>

This story presents each concept with a cast of four main characters, including the Lean, Python, Rust, and CUDA Rust programming languages.
This is because in the discipline of deep learning, the systems programming is *as* important as the mathematics.
The [Scaling Laws](https://gwern.net/scaling-hypothesis) — which we will soon come to intimately know — embody this principle,
and serve as the heart of this most recent wave (and arguably revolution) of artificial intelligence.
It's why current incarnations of deep learning frameworks like `torch` and `jax` have
modified their implementations to support distributed compilation.

Such mathematical concepts employed within the discipline of deep learning (and thus presented in SITP)
such as statistical distributions, high dimensional arrays, and optimizers
are based in the continuous mathematics of probability theory, linear algebra, and calculus.
This is because the
(combine ^^vv)
Throughout your journey, you will use the `teenygrad` deep learning framework in `Python`
to implement statistical machine learning and deep learning architectures,
take an intermission with the Intermezzo chapters by formalizing the same concepts with the interactive proof assistant `Lean`
— which can be optionally skipped upon a first reading —
and finally dive into the internals of the very same `teenygrad` primitives used by implementing your own `teenygrad`
with a mixture of `Python` (again) as well as the `Rust` and `CUDA Rust` programming languages.
By the end of our journey together, you will have a working implementation of `teenygrad` which is able to run the training and inference for `nanochat`.
You are encouraged to modify, extend, and hack on both thereafter.

Is it important to note that the various programming languages are the *means* of SITP rather than the *end*, in that we are *using* them as vehicles to implement
the end we actually do care about, which is that of `teenygrad` and `nanochat`.
Such vehicles are referred to as host languages, since our implementation of `teenygrad` is "hosted" in a mixture of
them<span class="sidenote-number"></span><span class="sidenote">*you could technically say that `teenygrad` is the "host language" of `nanochat`, but that is somewhat nonsesical due to their different natures of deep learning framework and neural network respectively, whereas the nature of `teenygrad` is closer to that of a programming language, even though it's a programming language for deep learning.*</span>.
...
model language subtly different. That is, as a *weighted set of sequences* with stochastic uncertainty
or <span class="defnote">**randomness**</span>**randomness**<span class="sidenote-number"></span><span class="sidenote">*There are few terms in discourse that involve lots of philosophical baggage. This is one of them, namely, whether randomness truly exists or not. Like most treatments of stochastics, we will expand on this when we get to the post-data inverse problem of statistics and differentiate between beliefs and frequencies.*</span>.
...
So when you ask systems like ChatGPT a <span class="defnote">**question**</span>**question**, it produces an <span class="defnote">**answer**</span>**answer** word by word by repeating the following inference loop.
...
> [!TIP]
> <img class="owl" src="./assets/owl.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Try completing the generation of the sentence by manually evaluating such loop by 1. clicking the predict button 2. choosing a word and 3. appending such word to the input box. <br style="clear: both;">
...
probability theory<span class="sidenote-number"></span><span class="sidenote">*As opposed to alternative frameworks such as such as probabilistic logic [https://en.wikipedia.org/wiki/Probabilistic_logic](https://en.wikipedia.org/wiki/Probabilistic_logic) or [uncertainty quantification](https://en.wikipedia.org/wiki/Uncertainty_quantification)*</span>.
...
> [!TIP]
> <img class="owl" src="./assets/owl.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Run code snippets by clicking the play button located at the top right. <br style="clear: both;">
