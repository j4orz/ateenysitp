...
However, the programmer interested in learning the skills necessary for the statistical learning approach to artificial intelligence
must make the transition made a few centuries ago by our mathematical cousins to the activity of modeling *continuous objects* with  *stochastic descriptions*.

As briefly mentioned in the [prologue](./pro.md), although the intellectual discipline of artificial intelligence is similar to those of neuroscience and psychology
in that they are all sciences of the mind, how artificial intelligence differs from the others is in the *methods* used to conduct it's experiments.
As the name suggests, artificial intelligence is a science of the artificial<span class="sidenote-number"></span><span class="sidenote">*Herbert Simon...*</span>
primarily relying on computation, since it can be directed and *designed* by a pattern of rules called a *program*.
That is, artificial intelligence — like **ChatGPT** — is "just" another computer program however implemented in a language called **PyTorch**.

In this wonderful whimsical whirwild tour called *The Structure and Interpretation of Tensor Programs*,
we will explore the aforementioned *continuous objects* and *stochastic descriptions*
which are used to program deep neural networks and deep learning frameworks, the most notable ones of course being ChatGPT and PyTorch.
However, because these two programs are quite complex (including lots of accidental complexity necessary for industrial codebases)
we will implement our very own chat assistant and deep learning framework capable of training such chat assistant.
That is, over the course of the next three booklets with
[Part I: Elements of Networks](), [Part II: Neural Networks](), and [Part III: Scaling Networks](),
we will implement (in an intertwined, spiraling fashion) the **[karpathy/nanogpt]()** net using
the [j4orz/teenygrad]() deep learning framework, and then the [j4orz/teenygrad]() framework itself.
Whether you're an eager high school student, an up and coming college student, or a battle-tested industry programmer,
the only prerequisite for *The Structure and Interpretation of Tensor Programs*
is that you have a solid grasp with the art of programming, and high school algebra.
...
<!-- 3. BLAS -->
- [1. Learning *Data* Distributions $p(X=x)$ from $D=\{x^{(i)}\}_{i=1}^{n}$]()
  ...
  - [1.5 Learning Probabilities from Data via Maximum Likelihood Estimation of Parameters]()
...
- [2. Learning *Function* Distributions $p(Y=y|X=x)$ from $D=\{(x^{(i)},y^{(i)})\}_{i=1}^{n}$]()
  - [2.1 From Distributions of Data $p(X=x)$ to Distributions of Functions $p(Y=y|X=x)$]()
  - [2.2 Predicting Scalars by Linearly Modeling Regression with $f: \reals^d \to \reals$]()
  - [2.3 Directly Fitting Linear Regression $f$ with Squared Error Loss $\mathscr{L(\theta)} := $]()
  - [2.4 Predicting Categories with Bigram Modeling by Counting Frequencies]()
  - [2.5 Predicting Categories by Linearly Modeling Classification with $f: \reals^d \to \N$]()
  - [2.6 Iteratively Fitting Logistic Regression $f$ with Cross Entropy Loss $\mathscr{L(\theta)} := $]()
...
such as ELIZA<span class="sidenote-number"></span><span class="sidenote">*Presented in [A Computer Program For the Study of Natural Language Communication Between Man and Machine. (Weizenbaum 1966)](https://dl.acm.org/doi/epdf/10.1145/365153.365168)*</span>
represented the meaning of words (referred to as semantics) was to simply use
...
So in the first three subchapters proper (not including the current chapter) we place our focus
...
```python
{{#include ../../teeny/examples/4.2-makemore-ffn1layer-torch.py:1:36}}
```

```python
{{#include ../../teeny/examples/4.2-makemore-ffn1layer-torch.py:42:44}}
```

```python
{{#include ../../teeny/examples/4.2-makemore-ffn1layer-torch.py:46:69}}
```

```python
{{#include ../../teeny/examples/4.2-makemore-ffn1layer-torch.py:77:78}}
```

```python
{{#include ../../teeny/examples/4.2-makemore-ffn1layer-torch.py:83:104}}
```
