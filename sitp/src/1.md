...
<img src="./assets/owl2.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Although separated by over 2000 years, the programmers of Silicon Valley face a daunting task
...
- [0. From Symbolic Software 1.0 to Stochastic Software 2.0](#0-from-logical-software-10-to-stochastic-software-20)
  ...
  - [0.2 Weizenbaum Cheats Turing's Test with the Pattern Matching of `ELIZA`](#02-weizenbaum-cheats-turings-test-with-the-pattern-matching-of-eliza)
  - [0.3 Wood's Interlingua with the Compilation of `LUNAR`](#03-woods-interlingua-with-the-compilation-of-lunar)
  ...
  - [0.4 Lenat's Advice Taker with the Frames of `CYC`](#04-lenats-general-advice-taker-with-the-frames-of-cyc)
  - [0.5 From A Logical to Distributional Semantics]() <!-- wittgenstein's bitter lesson -->
...
> [!WARNING]
> <img src="./assets/owl2.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Pause and think!<br style="clear: both;">
...
def eliza(input: str) -> str:
  ...
  elif match := re.fullmatch("Why does Alice hate me?", input): return "Why do you say Alice hates you?"
  else: raise NotImplementedError("")
```
...
> [!IMPORTANT]
> <img src="./assets/owl2.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Our mini `eliza()` will not qualitatively improve it's breadth of common sense nor it's depth of understanding
...
### 0.3 Wood's Interlingua with the Compilation of `LUNAR`
...
[Procedures as a Representaiton for Data in a Computer Program for Understanding Natural Language (Winograd 1971)]()
...
When questions become more difficult to think about from first principles perhaps such as this one,
one heuristic is to use a combination of both history and theories from other disciplines as a guide:
...
**What if we built a compiler for the English language?**<span class="sidenote-number"></span><span class="sidenote">*Not in the mood to implement a compiler for English in order to learn neural networks? We understand. Choose your own adventure by jumping now to [Chapter 1](#1-syntactic-sequence-learning-via-n-gram-models-with-teenygrad). You will miss out on an amazing philosophical insight made by one of Alan Turing's professors at the end of Chapter 0 however — sometimes smoking the whole pack can be good for you.<br><br> Additionally, having a rough idea of how they work from the 50,000ft view will help when we implement `teenygrad.compile()` in [Part III. Scaling Networks](), a Tensor compiler for neural networks. Don't say we didn't warn you!*</span><br>

</details>

That's effectively what the `SHRDLU` and `LUNAR`
systems<span class="sidenote-number"></span><span class="sidenote">*Presented across a series of papers. See [(Woods 1969)](), [(Woods 1970)](), [(Woods 1971)](), [(Woods 1972)]().*</span>
contemporaneously implemented.

`SHRDLU` built a question-answering natural language processing system to interface with a graphical blocks world.
`LUNAR` did the same to interface with a database containing chemical analysis data on lunar rock and soil from the from the Apollo moon missions.
Rather than query a database with a formal query language such as SQL,
`LUNAR` enables domain experts in geology to query such database with the natural language of English.
...
and the latter are English-like (whatever that may mean) such as Python.
Whatever is going underneath there, it seems like these languages know *some*thing.
Perhaps we can use the same techniques that formal programming languages use for *understanding* our intent
but extend them one level higher by applying them to the natural language of English itself?
In the case of `LUNAR`, geologists were able to express and prompt their questions to the system in English
without having to translate such questions into a formal query language like SQL.
...
literature<span class="sidenote-number"></span><span class="sidenote">*If you'd like a classic, we recommend Part X of the [Structure and Interpretation of Computer Programs (Abelson, Sussman, Sussman XYZ)](). For a modern yet still accessible introduction, you can't go wrong with [Programming Languages: Application and Interpretation (Krishmaurthi 2022)]()*</span>.

<!-- Let's take the example of translating a programming language expressed in C to one which is expressed in a machine language called x86. -->
The first idea to understand is that a compiler is a function which takes a string in and produces a string out.
The input string is referred to as the *source program* and the output string as the *target program*.
A traditional compiler might take in C as the source language and produce x86 as the target language.
In the case of `LUNAR` however, it will take in a question expressed in English and produce a query expressed in
SQL.

(todo, database is the processor (evaluator). go take a db course. maybe also quote woods BBN report that db is not the focus.
it's the syntactic and semantic analysis)
...
```python
def lunar(english_language: str) -> str:
  """translates a source question expressed in english to a target question expressed in SQL

  """
  target_query_language = ""
  return target_query_language
```
</div>

At a high level, a minimal `compile()`r follows a two step process:
1. It must first convert the source program from a <span class="defnote">**concrete syntax**</span>**concrete** `str` to
a more <span class="defnote">**abstract syntax**</span>**abstract** data structure that has more "understanding" of what's being spoken to them compared to a simple silly `str`. The simplest data structure one can use is a `Tree` in order to encode **hierarchy** (in which we will soon see why it's needed). Because at this point the program is now represented with a `Tree` and not the final x86 output which itself is a `str`, this abstract representation of the program is referred to as the <span class="defnote">**intermediate representation (IR)**</span>**intermediate representation**, or IR for short.
2. It must then convert the intermediate representation to the final target language.

The two conversion steps<span class="sidenote-number"></span><span class="sidenote">*With all industrial language implementations, there is almost always a third step in between the two, which optimizes the program. There is also almost always various IRs that successively get "lowered" to.*</span> have fancy names: *parsing* and *generating* respectively.
Converting such steps into Python functions we have the following:

<div class="dual">

```python
def compile(source_language: str) -> str:
  abstract_syntax = parse(source_language)
  target_language = generate(abstract_syntax)
  return target_language

def parse(concrete_syntax: str) -> Tree:
  raise NotImplementedError("")

def generate(abstract_syntax: Tree) -> str:
  raise NotImplementedError("")
```

```python
def lunar(english_language: str) -> str:
  logic_ir = parse(english_language)
  query_language = generate(logic_ir)
  return query_language

def parse(english_language: str) -> Tree:
  raise NotImplementedError("")

def generate(logic_ir: Tree) -> str:
  raise NotImplementedError("")
```

</div>

Following the [Design Recipe](https://docs.racket-lang.org/htdf/index.html), let's start with examples.

```python
def lunar(english_language: str) -> str:
  """
  >>> lunar("What's the average sample size of such")
    'XYZ'
  >>> lunar("Has anyone anlyzed rock 10046 for major elements")
    'XYZ'
  >>> lunar("Are any of the samples volcanics")
    'XYZ'
  >>> lunar("In which breccias is the average concentration of titanium greater than 6 percent")
    'XYZ'
  >>> lunar("How much titanium does S10017 contain")
    'XYZ'
  >>> lunar("Bulk chemistry of soil samples")
    'XYZ'
  >>> lunar("Which rock is the oldest")
    'XYZ'
  >>> lunar("Which is the oldest rock")
    'XYZ'
  """
  logic_ir = parse(english_language)
  query_language = generate(logic_ir)
...
Evaluating the code above fails as expected.
In the following two sections, we will implement the function bodies for `parse()` and `generate()` respectively so that the tests pass.


The next question the problem then reduces down to is
**what IR will you parse the source language of English *to*, and subsequently generate the target language of `SQL` *from*?**

> [!WARNING]
> <img src="./assets/owl2.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Pause and think!<br style="clear: both;">

**What if we parsed English to logic as an IR, and then generated SQL from said IR?**<br>
To no surprise, that's effectively what the `LUNAR` system does:

  <!-- """
  >>> lunar("What's the average sample size of such")
  '42'
  >>> lunar("Samples with silicon")
  'XYZ'
  >>> lunar("Which rocks do not contain chromite and ulvospinel")
  'XYZ'
  >>> lunar("Give me all lunar samples with magnetite")
  'XYZ'
  >>> lunar("Which elements has someone found in breccias and in basalts")
  'XYZ'

  >>> lunar("Analyses of strongtium in plagioclase")
  'XYZ'
  >>> lunar("I need all chemical analyses of lunar soil")
  'XYZ'
  >>> lunar("Has anyone anlyzed rock 10046 for major elements")
  'XYZ'

  >>> lunar("What are the igenous rocks")
  'XYZ'
  >>> lunar("Are any of the samples volcanics")
  'XYZ'

  >>> lunar("What is the average concentration of olivine in breccias")
  'XYZ'
  >>> lunar("What is the average age of the basalts")
  'XYZ'
  >>> lunar("In which breccias is the average concentration of titanium greater than 6 percent")
  'XYZ'
  ...
  >>> lunar("How much titanium does S10017 contain")
  'XYZ'
  >>> lunar("How much nickel is in rock 10046")
  'XYZ'

  >>> lunar("Bulk chemistry of soil samples")
  'XYZ'
  >>> lunar("Give me all references on fayalitic olivine")
  'XYZ'
  ...
  >>> lunar("Which is the oldest rock")
  'XYZ'
  >>> lunar("The highest titanium concentration")
  'XYZ'
  """ -->

#### 0.3.1 `parse(str): -> Tree` From Natural Language as Source to Formal Logic as IR
...
#### 0.3.2 `generate(Tree): -> str` From Formal Logic as IR to Formal Queries as Target

#### 0.3.3 `lunar(str): -> str` From Natural Language as Source to Formal Queries as Target
...
> [!IMPORTANT]
> <img src="./assets/owl2.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Although `lunar()`'s depth of understanding the natural language of English is deeper than `eliza()`s, 
...
> it will always be limited to the chemical analysis data in it's knowledge base from the Apollo moon missions.
...
We have presented the cliff notes of software 1.0's logical and finitely discrete methods:
- from psychology to artificial intelligence
- eliza pattern matcher
- lunar compiler
- cyc's ontology and inference
- wittgenstein's logical to distributional semantics
For more consult the resources in the bibliographic notes.
