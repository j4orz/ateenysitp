...
GPT-2 produces the following distribution with an `ndarray` of `.shape` `(10,)` which represents a vector with support in $\reals^{10}$).
...
As mentioned in the previous chapter, large language models are trained using methods from the discipline of <span class="defnote">**deep learning**</span>**deep learning**, which in turn are based in <span class="defnote">**machine learning**</span>**machine learning**,
...
hence why this most recent wave of methods in software 2.0 are coloquially known
...
was used in turn to generate full sentences with the inference loop.
...
The former is when you start with something and break it down whereas the latter is when you start with nothing and build it up.
...
(INSERT DIAGRAM OR VIDEO TO SIGNAL COMMA)
...
let us consult the classic interpretation of what a probability actually is.

- counting
- definition of conditinal probability from joint
- the two are related with product rule, or chain rule (point to sum rule)
