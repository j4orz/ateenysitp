...
to <span class="defnote defnote-stack">**software 2.0**</span>**software 2.0**<span class="sidenote-number"></span><span class="sidenote">*See [(Karpathy 2017)](https://karpathy.medium.com/software-2-0-a64152b37c35)*</span>, a distinction used to differentiate the classical act of programming software line by line,
...
- [0. From Symbolic Software 1.0 to Stochastic Software 2.0](#0-from-logical-software-10-to-stochastic-software-20)
  ...
  - [0.3 Wood's Interlingua with the Translation of `LUNAR`](#03-woods-interlingua-with-the-translation-of-lunar)
...
focused on building systems that <span class="defnote">**reasoning**</span>**reasoned** over an internal <span class="defnote defnote-stack">**representation**</span>**representation** of knowledge.
...
so that specific words within the input question can be**matched** via **template**
and subsequently **captured** so that `eliza` reuse such words in it's answer.
...
That is, it's safe to say that ELIZA utterly lacks any semantic understanding of word meaning.


(formal language theory. regular languages.)
a <span class="defnote">**regular language**</span>**regular language** are defined by regular expressions
a <span class="defnote defnote-stack">**regular expression**</span>**regular expression**
...
> [!IMPORTANT]
> <img src="./assets/owl2.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Our `eliza()` will not qualitatively improve it's breadth of common sense nor it's depth of understanding
...
### 0.3 Wood's Interlingua with the Translation of `LUNAR`
...
> [!WARNING]
> <img src="./assets/owl2.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Pause and think! When questions become more difficult to think about from first principles perhaps such as this one,
...
**What if we built a compiler for the English language?**<span class="sidenote-number"></span><span class="sidenote">*Not in the mood to implement a compiler for English in order to learn neural networks? We understand. Choose your own adventure by jumping now to [Chapter 1](#1-syntactic-sequence-learning-via-n-gram-models-with-teenygrad). You will miss out on an amazing philosophical insight made by one of Alan Turing's professors at the end of Chapter 0 however — sometimes smoking the whole pack can be good for you.<br><br> Additionally, having a quick compiler initiator or refresher will help when we implement `teenygrad.compile()` in [Part III. Scaling Networks](), a Tensor compiler for neural networks. Don't say we didn't warn you!*</span><br>

</details>

That's effectively what the `LUNAR`
system<span class="sidenote-number"></span><span class="sidenote">*Presented across a series of papers. See [(Woods 1969)](), [(Woods 1970)](), [(Woods 1971)](), [(Woods 1972)](), [(Winograd 1971)](https://apps.dtic.mil/sti/tr/pdf/AD0721399.pdf).*</span>
does, building a natural language interface with a database containing chemical analysis data on lunar rock and soil from the from the Apollo moon missions
...
For instance, "give me all analyses of S10046.", rather than `SELECT * WHERE (todo)`

#### 0.3.1 Natural Language Translation
...
Whatever is going underneath there, it seems like these languages *understand* at least *some*thing
in order to translate our intent from a higher level language to a lower level one.
...
`LUNAR` takes in a question expressed in English and produces a query expressed in SQL.
...
In order to produce a final answer you will need a program <span class="defnote">**evaluator**</span>**evaluator** for x86, which is any Intel processor which that understands x86.
Similarly with `LUNAR`, after translating the query from English to SQL, a database that understands SQL is
...
def lunar(english_source: str) -> str:
  """translates a source question expressed in english to a target question expressed in SQL
  >>> lunar("Give me all analyses of S10046")
    '42'
  """
  sql_target = ""
  return sql_target
...
Now that we have an idea of what the expected input output data looks like, what about the function?
At a high level, a minimal `compile()`r follows a two step process:
1. It first converts the source program from a <span class="defnote">**concrete syntax**</span>**concrete** `str` to
a more <span class="defnote defnote-stack">**abstract syntax**</span>**abstract** data structure that has more "understanding" of what's being spoken to it compared to a simple silly `str`. Because this abstract representation of the program is not the final representation (namely x86), it's referred to as the <span class="defnote">**intermediate representation (IR)**</span>**intermediate representation**, or IR for short.
...
We have the following after converting such steps into Python:
...
def parse(concrete_syntax: str) -> IR:
  raise NotImplementedError("")

class IR():
  def __new__(): return None

def generate(abstract_syntax: IR) -> str:
  raise NotImplementedError("")
```

```python
def lunar(english_source: str) -> str:
  abstract_syntax = parse(english_source)
  sql_target = generate(abstract_syntax)
  return sql_target

def parse(english_source: str) -> IR:
  raise NotImplementedError("")
...
Now that we've decomposed `lunar(str): -> str` into `parse(str): -> IR` and `generate(IR): -> str`,
let's implement their implementations "incrementally" to support new language features.


#### 0.3.2 The Parser Lifts the Linguistics


starting with compiling `"give me all analyses of S10046."` to `MRL (todo)`
...
**What if we parsed English to XYZ as an IR, and then generated ABC from said IR?**<br>
To no surprise, that's effectively what the `LUNAR` system does.

</details>

(formal language theory. context-free languages.)
...
a <span class="defnote defnote-stack">**context-free grammar**</span>**context-free grammar**
loremipsumloremipsumloremipsumloremipsumloremipsumloremipsumloremipsumloremipsumloremipsumloremipsum
a <span class="defnote defnote-stack">**derivation**</span>**derivation**
loremipsumloremipsumloremipsumloremipsumloremipsumloremipsumloremipsumloremipsumloremipsumloremipsum
a <span class="defnote defnote-stack">**parser**</span>**parsing** is the problem of finding a derivation for a string in a grammar (recognizer)


#### 0.3.3 The Generator Lowers the Logic

#### 0.3.4 The Standard Implementation Plan
...
real-world problems. Although not comprehensive, we explored the flavor of
logically and discretely finite methods that the software 1.0 approach to AI employed
...
were [(Weizenbaum 1966)](https://dl.acm.org/doi/epdf/10.1145/365153.365168),
[(Minsky 1968)]() [(Winograd 1971)](), and [(Woods 1972)]().
...
as <span class="defnote">**connectionism**</span>**connectionism** and <span class="defnote defnote-stack">**symbolism**</span>**symbolism**. 
