...
- [1. Sequence Learning](#1-sequence-learning)
  ...
  - [1.2 Recovering the Distribution of Language](#12-recovering-the-distribution-of-language)
...
As you now know from [Chapter 1. Sequence Learning](#1-sequence-learning), language models are next token predictors.
...
Before we dive into the internals of [`teenygrad`](https://github.com/j4orz/ateenysitp) itself in [Chapter 2. From IPL's Array to APL's Multidimensional Array](#2-from-ipls-array-to-apls-multidimensional-array) and implement our own framework capable of training the bigram and logistic regression models,
we will formally characterize our language model implementations from Chapter 1 using the language of probability, linear algebra, and calculus.

<div class="toc">

- [Intermezzo One: The Language of Probability and Matrix Calculus]()
  - [I1. Measure Theoretic Probability Theory]()
    - [I1.1 Probability Spaces]()
    - [I1.2 Random Variables and their Distributions]()
    - [I1.3 Random Processes and their Kernels]()
    - [I1.4 Language Models as Locally Normalized Kernels]()
    - [I1.5 Language Models as Globally Normalized Measures]()
  - [I2. Matrix Calculus]()
...
Before we can formally define a probability distribution, we must first define the probability space, which corresponds to the picture below.

<img class="probspace" src="./assets/probspace.png" alt="" style="display: block; margin: 0 auto;">

This picture is comprised of three components. Namely, the blue box and it's blue dots, the colored boxes, and the mapping from the colored boxes to the number line:
...
3. probability law $\mathbb{P}$: a mapping from the event space to a number

> [!CAUTION]
> <img class="owl" src="./assets/owl.png" alt="" style="float: left; width: 140px; margin: 0 1em 0.5em 0;"> Probability spaces are usually NOT implemented with code. That is, we do NOT instantiate Python `set()`s for the sample space $\Omega$, the event space $\mathcal{F}$, nor define some function `def probability_law(event_space: set) -> float` for $\mathbb{P}$. The purpose of the notion of probability spaces is 1. conceptual clarity with definitions 2. generality with abstract definitions<span class="sidenote-number"></span><span class="sidenote">*The generality of mathematical concepts are usually "more" general than even concepts that might be implemented with type polymorphism. For instance, this abstract definition of a probability space is generalized to cover any random experiment we'd like to model, from rolling dice, to generating language. However it's quite unusual to have a program need to progam both.*</span> and 3. rigorous results with theorems. What *does* usually get implemented are probability distributions, for instance with `np.random.todo` We will connect the concept of probability spaces with probability distributions shortly up ahead.

Let us now define each component.

<div class="definition" data-title="Definition: Sample Space">

A **sample space** is the set of all possible outcomes in an experiment, and is denoted with $\Omega$ ($\LaTeX$ code: `\Omega`).

</div>

<div class="lemma" data-title="Example: Sample Space">
...
A **event space** is the set of all possible subsets of the sample space, and is denoted with $\mathcal{F}$ ($\LaTeX$ code: `\mathcal{F}`).

</div>

<div class="lemma" data-title="Example: Event Space">
...
<div class="lemma" data-title="Example: Probability Law">
