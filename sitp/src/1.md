...
- [0. From Symbolic Software 1.0 to Stochastic Software 2.0](#0-from-logical-software-10-to-stochastic-software-20)
  ...
  - [0.3 Wood's Winograd Challenge with the Translation of `LUNAR`](#03-woods-winograd-challenge-with-the-translation-of-lunar)
...
- [*Intermezzo One: The Language of Set Theory*](#intermezzo-one-the-language-of-dependent-type-theory)
- [1. Syntactic Sequence Learning with n-gram Models](#1-syntactic-sequence-learning-with-n-gram-models)
  - [1.1 From Certain to Uncertain Knowledge](#11-from-certain-to-uncertain-knowledge) <!-- Probability -->
  - [1.2 Probability as Array Programming](#12-probability-as-array-programming)
  - [1.3 Statistics as Inverse Probability]() <!-- Statistics -->
  - [1.6 Summary]()
...
- [*Intermezzo Two: The Language of Probability Theory and Statistics*]()
- [2. Semantic Sequence Learning with Linear Models]()
...
- [*Intermezzo Three: The Language of Linear Algebra and Matrix Calculus*]() <!-- linear regression -->
- [3. From Abstract to Numerical Linear Algebra](#3-from-abstract-to-numerical-linear-algebra)
  - [3.1 From Virtual to Physical Machines](#31-from-virtual-to-physical-machines) <!-- three language problem: rust translation and risc-v evaluation -->
  - [3.2 From Virtual to Physical Shapes](#32-from-virtual-to-physical-shapes) <!-- strides and layouts -->
  - [3.3 Accelerating Basic Linear Algebra Subroutines]()
  - [3.4 Accelerating Communication with Hierarchies]()
  - [3.5 Accelerating Computation with Pipelines]()
  - [3.6 Summary]()
...
## *Intermezzo One: The Language of Set Theory*
...
...
## 1. Syntactic Sequence Learning with n-gram Models <!-- $p(X=x)$ from $D=\{x^{(i)}\}_{i=1}^{n}$ -->
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
(INSERT nanochat EXAMPLE with logitlens/xformer lens)
...
we can evaluate <span class="defnote">**language model**</span>**language models** like `nanochat` which one has a higher <span class="defnote">**chance**</span>**chance** of occuring?
...
&\dots \\
p(\text{xray}|\text{Colorless green ideas sleep})&=0.00001 \\
p(\text{yak}|\text{Colorless green ideas sleep})&=0.00001 \\
p(\text{zuchini}|\text{Colorless green ideas sleep})&=0.00001 \\
\end{aligned}
$$

in which `nanochat` samples the most likely word "furiously".
...
it produces an <span class="defnote">**answer**</span>**answer** word by word by repeating the following loop:

1. evaluating the probability of the next word
2. sampling such word,
3. appending it to the existing text

So with all that said. The first task in our journey to implementing an artificially intelligent conversational machine like `nanochat`
is to formalize our intuitions around the notions of uncertainty, chance, and likelihood. We will do so with the language of probability theory.

### 1.2 Probability as Array Programming
In the previous subchapter, we mentioned that the statistical disciplines of machine learning and deep learning
model language (hence language model) as a *weighted set of strings* with stochastic uncertainty.
To formalize our intuitions around uncertainty, chance, and likelihood, we need the language of probability
theory<span class="sidenote-number"></span><span class="sidenote">*As opposed to alternative frameworks such as such as [probabilistic logic](https://en.wikipedia.org/wiki/Probabilistic_logic) or [uncertainty quantification](https://en.wikipedia.org/wiki/Uncertainty_quantification)*</span>.

<!-- probability space -->
- sample space (set of outcomes)
- event space (set of subsets)
...
### 1.3 Statistics as Inverse Probability
...
## 2. Semantic Sequence Learning via Linear Models with `teenygrad` <!-- $p(Y=y|X=x)$ from $D=\{(x^{(i)},y^{(i)})\}_{i=1}^{n}$ -->
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
why deep learning? representation learning.

the machinery has been there since the 80's
running backprop on a feed forward neural network.
although yes, the inductive bias of transformers for sequence learning allows XYZ compared to bengio et al., 2003
the primary driver is data and scale.
...
### 3.1 From Virtual to Physical Machines
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
### 3.2 From Logical to Physical Shapes
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
...
### 3.4 Accelerating Communication with Hierarchies via Loop Reordering, Register and Cache Blocking

### 3.5 Accelerating Computation of `GEMM` with Instruction Level Parallelism via Loop Unrolling
<small>[$\hookleftarrow$ Table of Contents](#table-of-contents)</small>
<!-- loop unroll -->
...
### 3.6 From Abstract to Numerical Linear Algebra
...
## 4. From Abstract to Numerical Linear Algebra
